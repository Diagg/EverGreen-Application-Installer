...
If ($DisableUpdate -eq $true){Write-log "Action requested: Disabling update feature"}
...
If(($Uninstall -ne $true) -and [String]::IsNullOrWhiteSpace($InstallSourcePath))
    {     
        ...
        Try
            {
                If ([version]((Get-PackageProvider -Name 'nuget' -ErrorAction SilentlyContinue).version) -lt [version]"2.8.5.201")
                    {
        ...
        Try
            {
                ...
                $AppInfos = Get-GoogleChrome|Where-Object Architecture -eq $Architecture
            }
        Catch
